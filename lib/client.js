window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			activateProvider: "激活 Provider",
			noneOption: "不使用",
			tavilyOption: "Tavily — 高质量 AI 搜索",
			baiduOption: "百度搜索 — 国内搜索",
			configLabel: "配置",
			unsavedChanges: "● 有未保存的更改",
			saveFailed: "✗ 保存失败",
			providerChangedMsg: "✓ 已保存，切换 Provider 需重启 DSH 生效",
			connectionOk: "✓ 连接成功",
			connectionFail: "✗ HTTP",
			notConfigured: "网络搜索未启用",
			notConfiguredHint: "选择 Tavily 或百度搜索启用 web_search 工具",
		};
		...
		const en = {
			...
			noneOption: "None",
			tavilyOption: "Tavily — High-quality AI Search",
			baiduOption: "Baidu Search",
			configLabel: "Config",
			unsavedChanges: "● Unsaved changes",
			saveFailed: "✗ Save failed",
			providerChangedMsg: "✓ Saved. Restart DSH to switch provider.",
		...
		function WebSearchSettingsTab({ t }) {
			...
			function doSave() {
				...
				fetch('/plugins/web-search-providers/api/config', {
					...
					if (r.ok) {
						setMsg(providerChanged ? t('providerChangedMsg') : t('saved'));
						setDirty(false);
			...
			function doTest() {
				...
				if (provider === 'tavily') {
					...
					fetch(url, {
						...
						setTestResult(r.ok ? t('connectionOk') : t('connectionFail') + ' ' + r.status);
						setTesting(false);
			...
			return js("div", { style: { padding: "16px 0", display: "flex", flexDirection: "column", gap: "16px" }, children: [
				...
				js("div", { style: { ...cardStyle, padding: "20px 24px" }, children: [
					...
					field(t("activateProvider"), j("select", {
