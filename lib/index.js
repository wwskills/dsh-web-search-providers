...
function apply(ctx, config = {}) {
  ...
  console.log('[web-search-providers] loaded config: activeProvider=' + (cfg.activeProvider || 'none') + ', tavily.apiKey=' + (cfg.tavily && cfg.tavily.apiKey ? cfg.tavily.apiKey.slice(0,8) + '...' + cfg.tavily.apiKey.slice(-4) : 'MISSING'));
  ...
  function resolveOptions() {
    ...
    if (p === 'tavily') {
      ...
      console.log('[web-search-providers] resolveOptions tavily: apiKey=' + (c.apiKey ? c.apiKey.slice(0,8) + '...' + c.apiKey.slice(-4) : 'MISSING') + ', cfg keys=' + Object.keys(state.cfg).join(','));
      return {
        apiKey: c.apiKey || '',
  ...
  function registerActiveProvider() {
    ...
    if (_opts) {
      var _k = _opts.apiKey || '';
      console.log('[web-search-providers] provider apiKey=' + (_k ? _k.slice(0,8) + '...' + _k.slice(-4) : 'MISSING'));
    }
    } catch (e) {
