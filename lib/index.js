...
function apply(ctx, config = {}) {
  ...
  // Use ctx.effect to tie provider lifecycle to this plugin's fiber,
  // NOT to the temporary inject callback fiber.
  ctx.inject(['web'], (webCtx) => {
    state.webCtx = webCtx;
    registerActiveProvider();
    // Keep the fiber alive — the disposer registered by registerSearchProvider
    // will be cleaned up when THIS plugin is unloaded, not when inject returns.
    return () => {
      if (state.currentDisposer) {
        try { state.currentDisposer(); } catch {}
        state.currentDisposer = null;
