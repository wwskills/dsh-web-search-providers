...
function apply(ctx, config = {}) {
  ...
  function registerActiveProvider() {
    ...
    try {
      ...
      // Pin DSH's searchProviderId to our active provider so it takes priority
      // over DSH's built-in deepseek-official provider.
      state.webCtx.web.searchProviderId = state.activeProvider;
  ...
  const registerWebRoutes = () => {
    ...
    // Test connection endpoint (server-side, avoids browser CORS)
    ws.register({
      kind: 'exact',
      path: '/plugins/web-search-providers/api/test',
      handler: async (req, res) => {
        if (req.method !== 'POST') {
          res.writeHead(405);
          res.end('Method not allowed');
        ...
        try {
          ...
          if (provider === 'tavily') {
            const c = state.cfg.tavily || {};
            const baseURL = c.baseURL || TAVILY_DEFAULT_BASE_URL;
            const resp = await fetch(`${baseURL}/search`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ api_key: c.apiKey, query: 'test', max_results: 1 }),
            });
            res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' });
            res.end(JSON.stringify({ ok: resp.ok, status: resp.status }));
          } else if (provider === 'baidu') {
            const c = state.cfg.baidu || {};
            const resp = await fetch(`${BAIDU_DEFAULT_ENDPOINT}/v2/ai_search/web_search`, {
              method: 'POST',
              headers: {
    ...
    }), 'web-search-providers: test-api';

    // Usage stats endpoint
    ws.register({
      kind: 'exact',
      path: '/plugins/web-search-providers/api/usage',
      handler: async (req, res) => {
        if (req.method !== 'GET') {
          res.writeHead(405);
          res.end('Method not allowed');
    ...
    }), 'web-search-providers: usage-api';
  };
